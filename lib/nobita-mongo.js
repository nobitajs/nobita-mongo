...
const bind = config => {
  ...
  for (let table in config.tables) {
    _f = _.merge(_f, create(databaseName, table, config.tables[table]));
  }
  return _f;
}
...
const create = (databaseName, tableName, schema) => {
  ...
  _f[tableName] = {
    ...
    async findOne(sql, data = {}) {
      try {
        let res = await model[tableName].findOne(sql, data.filter, data);
        res = res ? res.toObject() : res
        return {
          code: 200,
          data: res,
          msg: 'success'
    ...
    async update(sql, newDate, params) {
      const data = await model[tableName].find(sql);
